...
struct JSON {
    ...
    JSON at_path(const std::string& path) const {
        ...
        while(i < path.size()) {
            ...
            if(std::isalpha(path[i]) || path[i]=='_') {
                ...
                if(!current->is_object()) return JSON(); // Return null instead of throwing
    ...
    // JSON Path modification - the missing piece!
    void set_path(const std::string& path, const JSON& val) {
        JSON* current = this;
        size_t i = 0;
        std::vector<std::pair<std::string, int>> path_parts; // key and index (-1 for object key)
    ...
    bool operator<(const JSON& other) const {
        // First, compare by type index. This provides a consistent ordering for different types.
        if (value.index() != other.value.index()) {
            return value.index() < other.value.index();
        }

        // If the types are the same, visit the values to compare them.
        return std::visit(
            [](const auto& lhs, const auto& rhs) -> bool {
                
                if constexpr (std::is_same_v<decltype(lhs), decltype(rhs)>) {
                    // This block is only compiled for pairs of the same type.
                    using T = std::decay_t<decltype(lhs)>;

                    if constexpr (std::is_same_v<T, std::nullptr_t>) {
                        return false; // Two nulls are equal, not less than.
                    } else {
                        // All other same-type pairs are comparable with operator<.
                        return lhs < rhs;
                    }
                } else {
                    // This block is compiled for pairs of different types.
                    // It will never be executed at runtime because of the index check above.
                    // We must provide a valid return statement to satisfy the compiler.
                    return false;
                }
            },
            value, other.value
    ...
    // ============ SERIALIZATION ============
    std::string dump(bool pretty = false, int indent = 0, int max_precision = 6) const {
        ...
        if (is_null()) { 
            ...
            for (const auto& el : arr) {
                ...
                first = false;
                if (pretty) oss << "\n" << std::string(indent+2,' ');
                oss << el.dump(pretty, indent+2, max_precision);
            }
            ...
            for (const auto& [k,v] : obj) {
                ...
                first = false;
                if (pretty) oss << "\n" << std::string(indent+2,' ');
                oss << '"' << k << "\":" << (pretty ? " " : "") << v.dump(pretty, indent+2, max_precision);
            }
    ...
    std::string dump_pretty(int indent_size = 2) const { return dump(true, 0, 6); }
...
auto name = json.at_path("user.profile.name").as_string();

// File I/O:
json.to_file("data.json");
auto loaded = ejson::JSON::from_file("data.json");
...
for (auto& item : json["scores"]) {
    std::cout << item.as_number() << std::endl;
}
